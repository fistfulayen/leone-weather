...
export async function GET() {
  try {
    console.log('Testing narrative generation (debug mode)...');
...
Sign off: "— Louisina 🦁"`;
    ...
    console.log('Test narrative generated successfully (not stored in database)');

    // Return the narrative for review without storing in database
    return NextResponse.json({
      success: true,
      message: 'Test narrative generated (not stored in database)',
      narrative,
      metadata: {
        date: todayDate,
    ...
    console.error('Error testing narrative generation:', error);
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to generate test narrative',
      },
      { status: 500 }
    );
  }
