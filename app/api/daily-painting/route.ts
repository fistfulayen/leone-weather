...
Return ONLY the image generation prompt, nothing else.`;
    ...
    // Generate the painting using Gemini 3 Pro Image (Nano Banana Pro)
    ...
    if (!imageData) {
      console.error('No image generated by Gemini. Response:', JSON.stringify(geminiResult.response, null, 2));
      return NextResponse.json({
        painter: todaysPainter,
        sourceImage: selectedImage,
        prompt: imagePrompt,
        imageUrl: null,
        error: 'Image generation failed: No image data returned from Gemini',
      }, { status: 500 });
    }

    console.log('Image generated successfully by Gemini');

    // Convert base64 image data to buffer (Gemini returns base64 directly)
    const downloadedImageBuffer = Buffer.from(imageData, 'base64');
