...
export async function GET(request: Request) {
  try {
    ...
    const includeCrypto = ['crypto', 'nfts', 'newsonly', 'paintingonly', 'news', 'painting', 'full'].includes(testMode);
    if (includeCrypto) {
    try {
    ...
    let localNews = null;
    const includeNews = ['newsonly', 'paintingonly', 'news', 'painting', 'full'].includes(testMode);
    if (includeNews) {
    try {
    ...
    const includePainting = ['paintingonly', 'painting', 'full'].includes(testMode);
    if (includePainting) {
    try {
  ...
  <div style="background: #ffffff; border-radius: 12px; padding: 24px; margin-bottom: 24px; border: 1px solid #e5e7eb;">
    ...
    ${cryptoPunksSales.map((sale: any) => `
      <div style="display: flex; align-items: flex-start; gap: 16px; padding: 16px; background: #f9fafb; border-radius: 8px; border: 1px solid #e5e7eb; margin-bottom: 12px;">
        <img
          src="${sale.punkId <= 9999 ? `https://www.larvalabs.com/public/images/cryptopunks/punk${sale.punkId}.png` : sale.imageUrl}"
